    ...
    <div className="card" style={{ marginBottom: 12 }}>
      <div className="ch">
        <div className="ch-l">
          ...
          <div>
            <div className="ch-title">일별 판매 추이 (매출 · 광고 vs 오가닉)</div>
            <div className="ch-sub">
              {chartFrom} ~ {chartTo} · {merged.length}일 ·
              {` ${attrWindow === '14d' ? '14일' : '1일'} 어트리뷰션 · ${VAT_LABEL}`}
              {lastUploadDate && ` · CSV 최신: ${lastUploadDate}`}
              {loading && ' · 불러오는 중...'}
            </div>
          </div>
        </div>
      ...
      <div className="cb">
        {/* KPI 카드 — 모드 무관하게 항상 표시 (광고/오가닉 매출 분석은 항상 의미 있음) */}
        ...
        {showEmpty ? (
          ...
          /* Stacked Bar (오가닉 + 광고) — 모드 무관하게 항상 매출 기준 */

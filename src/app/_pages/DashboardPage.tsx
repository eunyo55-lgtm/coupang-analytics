...
export default function DashboardPage() {
  ...
  const yoyChartData = useMemo(() => {
    ...
    state.daily26.forEach(r => {
      if (r.date >= chartFrom && r.date <= chartTo) map26.set(r.date, r.qty)
    })
    state.daily25.forEach(r => {
      const mapped = r.date.replace('2025','2026')
      const d25 = chartFrom.replace('2026','2025')
      const d25e = chartTo.replace('2026','2025')
      if (r.date >= d25 && r.date <= d25e) map25.set(mapped, r.qty)
    })
    state.daily24.forEach(r => {
      const mapped = r.date.replace('2024','2026')
      const d24 = chartFrom.replace('2026','2024')
      const d24e = chartTo.replace('2026','2024')
  ...
  const kpiCards = [
    {
      label: '판매량', sub: `전일 (${latestDate})`,
      qty: kpiYest?.qty ?? null, rev: kpiYest?.rev ?? null,
      yoy: kpiYest && kpiYest25 ? pct(kpiYest.qty, kpiYest25.qty) : null,
      color: 'var(--blue)',
    },
    {
      label: '주간 판매량', sub: `${weekRange.from.slice(5)} ~ ${weekRange.to.slice(5)} (금~목)`,
      qty: kpiWeek?.qty ?? null, rev: kpiWeek?.rev ?? null,
      yoy: kpiWeek && kpiWeek25 ? pct(kpiWeek.qty, kpiWeek25.qty) : null,
      color: 'var(--purple)',
    },
    {
      label: '누적 판매량', sub: `26년 (${cumRange.from.slice(5)} ~ ${latestDate.slice(5)})`,
      qty: kpiCum?.qty ?? null, rev: kpiCum?.rev ?? null,
      yoy: kpiCum && kpiCum25 ? pct(kpiCum.qty, kpiCum25.qty) : null,
      color: 'var(--green)',
    },
    {
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="ch">
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <input type="date" value={chartFrom}
              onChange={e => setChartFrom(e.target.value)}
              style={{ fontSize:11, padding:'4px 8px', borderRadius:6, border:'1px solid var(--border)', background:'var(--bg)', color:'var(--text)' }}
            />
            <span style={{ fontSize:11, color:'var(--t3)' }}>~</span>
            <input type="date" value={chartTo}
          ...
          {yoyChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={330}>
              <LineChart data={yoyChartData} margin={{ top:8, right:20, left:0, bottom:5 }}>
                ...
                <Tooltip
                  formatter={(val: number, name: string) => [fmt(val) + '개', name]}
            ...
            <div className="empty-st" style={{ height: 330 }}>
                ...
                <tbody>
                  {topProducts.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ fontWeight:700, maxWidth:130, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.product_name}</td>
                      <td style={{ fontWeight:700 }}>{fmt(item.total_qty)}</td>
                      <td style={{ fontWeight:800, color:'var(--blue)', fontSize:11 }}>{fmt(item.total_revenue)}</td>
                    </tr>
                  ))}
                </tbody>
              </table></div>
      ...
      <div className="card">
        <div className="ch">
          ...
          <button onClick={() => { const nav = (window as Record<string,unknown>).navigateTo as ((p:string)=>void); nav?.('/ad') }}

...
function SalesTrendModal({ productName, onClose }: { productName: string; onClose: () => void }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:999,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={onClose}>
      <div style={{background:'var(--card)',borderRadius:'var(--r12)',padding:24,width:'min(720px, 95vw)',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
          ...
          <button onClick={onClose} style={{background:'var(--bg)',border:'1px solid var(--border)',borderRadius:8,padding:'6px 12px',cursor:'pointer',fontSize:12,color:'var(--t2)'}}>✕ 닫기</button>
        </div>
        {trendData.length>0?(
...
function StockTrendModal({ productName, stockHistory, onClose }: { productName:string; stockHistory:{week:string;qty:number}[]; onClose:()=>void }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:999,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={onClose}>
      <div style={{background:'var(--card)',borderRadius:'var(--r12)',padding:24,width:'min(600px, 95vw)',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
...
export default function DashboardPage() {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      const merged=(p26||[]).map((r:TopProduct)=>({...r,qty_26:r.total_qty,qty_25:map25.get(r.product_name)||0,qty_24:map24.get(r.product_name)||0}))
      setTopProducts(merged);setTopStock(stocks||[]);setLoadingTop(false)
    })
  },[topFrom,topTo])
  ...
  const yoyChartData=useMemo(()=>{
    ...
    state.daily25.forEach(r=>{const m=r.date.replace('2025','2026'),d25=chartFrom.replace('2026','2025'),d25e=chartTo.replace('2026','2025');if(r.date>=d25&&r.date<=d25e)map25.set(m,r.qty)})
    state.daily24.forEach(r=>{const m=r.date.replace('2024','2026'),d24=chartFrom.replace('2026','2024'),d24e=chartTo.replace('2026','2024');if(r.date>=d24&&r.date<=d24e)map24.set(m,r.qty)})
  ...
  const supplyChartData=useMemo(()=>{
    ...
    orders.forEach(r=>{
      const dateStr=String(r['order_date']||'');if(!dateStr)return
    ...
    return Array.from(monthMap.entries()).sort(([a],[b])=>a.localeCompare(b)).slice(-12)
      .map(([month,v])=>({month:month.slice(2),'발주수량':v.order,'공급수량':v.confirmed,'입고수량':v.received}))
  },[state.ordersData])
  ...
  return (
    <div>
        ...
        {kpiCards.map((c,i)=>(
          <div key={i} className={`ds-card ds-c${i+1}`}>
            ...
            <div style={{fontSize:11,fontWeight:700,color:'var(--t2)',margin:'3px 0'}}>
              {(c as {isStock?:boolean}).isStock?`재고액 ${c.rev!==null?Math.round((c.rev||0)/100000000*10)/10+'억':'—'}`:`매출 ${c.rev!==null?fmt(c.rev||0)+'원':'—'}`}
            </div>
      ...
      <div className="card" style={{marginBottom:12}}>
        <div className="ch">
          <div className="ch-l"><div className="ch-ico">🚚</div><div><div className="ch-title">공급 현황</div><div className="ch-sub">월별 발주수량 · 공급수량 · 입고수량</div></div></div>
        </div>
        <div className="cb">
      ...
      <div className="g2" style={{marginBottom:12}}>
                    ...
                    {topProducts.map((item,i)=>{
                      ...
                      return(
                        <tr key={i}>
                          ...
                          <td><span onClick={()=>setSalesModal(item.product_name)} style={{fontWeight:700,maxWidth:110,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',display:'block',cursor:'pointer',color:'var(--blue)',textDecoration:'underline dotted'}} title="클릭 시 판매 추이">{item.product_name}</span></td>
        ...
        <div className="card">
          <div className="ch">
            <div className="ch-l"><div className="ch-ico">📦</div><div><div className="ch-title">재고 TOP 10</div><div className="ch-sub">상품명 기준 바코드 합산</div></div></div>
          </div>
          <div className="cb" style={{padding:'4px 14px 10px'}}>
            {topStock.length>0?(
              <div className="tw" style={{overflowX:'auto'}}>
                    ...
                    {topStock.map((p,i)=>{
                      ...
                      return(
                        <tr key={i}>
                          ...
                          <td><span onClick={()=>setStockModal({name:p.product_name,history:[]})} style={{fontWeight:700,maxWidth:100,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',display:'block',cursor:'pointer',color:'var(--blue)',textDecoration:'underline dotted'}} title="클릭 시 재고 추이">{p.product_name}</span></td>

...
function toD(s: string) { return s ? String(s).slice(0, 10) : '' }
...
export default function SupplyPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // products에서 barcode → cost, image_url 매핑 (200개씩 배치)
      const barcodes = [...new Set((Array.isArray(data) ? data : []).map(r => r['SKU Barcode']).filter(Boolean))]
  ...
  const filtered = useMemo(() => {
    return rows.filter(r => {
      ...
      const inSearch = !search || r['SKU 이름'].toLowerCase().includes(search.toLowerCase()) || r['SKU Barcode'].includes(search)
  ...
  function calcKpi(rowSet: SupplyRow[]) {
    ...
    const amt = rowSet.reduce((s, r) => s + r.확정수량 * (r.cost || 0), 0)
    return { qty, amt }
  }
  ...
  // KPI: 입고예정일 기준
  const kpiYest = useMemo(() => calcKpi(rows.filter(r => toD(r.입고예정일) === yesterdayStr)), [rows, yesterdayStr])
  ...
  const kpiMoving = useMemo(() => calcKpi(rows.filter(r => toD(r.입고예정일) >= today && toN(r.입고수량) === 0)), [rows, today])

  // 미납 리스트: 입고수량 < 확정수량
  const unpaid = useMemo(() => filtered.filter(r => r.입고수량 < r.확정수량), [filtered])

  // 이동중 파이프라인: 입고예정일 >= 오늘 AND 입고수량 = 0, 일별 그룹
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="ch">
          <div className="ch-l"><div className="ch-ico">🚚</div><div><div className="ch-title">공급 현황</div><div className="ch-sub">입고예정일 기준</div></div></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
              style={{ fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)' }} />
            <input className="si" placeholder="🔍 상품명/바코드 검색" value={search} onChange={e => setSearch(e.target.value)} style={{ width: 180 }} />
          </div>
        </div>
        <div className="cb">
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="ch">
          <div className="ch-l"><div className="ch-ico">⚠️</div><div>
            ...
            <div className="ch-sub">입고수량 &lt; 확정수량 ({unpaid.length}건)</div>
          </div></div>
        </div>
        <div className="cb">
          <div className="tw" style={{ overflowX: 'auto' }}>
                ...
                {unpaid.length > 0 ? unpaid.map((r, i) => {
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td style={{ fontWeight: 700, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r['SKU 이름']}</td>
                      <td style={{ fontSize: 11, color: 'var(--t3)' }}>{r['SKU Barcode']}</td>
                      <td style={{ textAlign: 'right' }}>{fmt(r.발주수량)}</td>
                      <td style={{ textAlign: 'right', color: 'var(--blue)' }}>{fmt(r.확정수량)}</td>
                      <td style={{ textAlign: 'right', color: 'var(--green)' }}>{fmt(r.입고수량)}</td>
        ...
        <div className="cb">
          {moving.length > 0 ? (
            ...
            <div className="empty-st">
              ...
              <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 4 }}>입고예정일이 오늘 이후이고 입고수량이 0인 상품이 없습니다</div>
            </div>
          )}
        </div>
      </div>
